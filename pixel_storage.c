...
struct pixel_entry *insert_entry(struct pixel node) {
    ...
    do {
        ...
        found_place = (entry->pixel_node->y <= head_entry->pixel_node->y && entry->pixel_node->x <= head_entry->pixel_node);
...
int draw_screen(struct pixel_entry *head_draw_entry) {
    // TODO: move and improve main function drawing code
}
